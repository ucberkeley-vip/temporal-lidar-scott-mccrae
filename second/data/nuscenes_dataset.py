...
class NuScenesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        examples = []
        input_dict_list = self.get_sensor_data(query=idx, sweepsPerPC=3)
    ...
    # Note: sweepsPerPC specifies how many lidar sweeps should be in each point cloud in the list returned
    # added by SM
    # this should be passed in by __getitem__
    def get_sensor_data(self, query, sweepsPerPC=1, maxSweeps=9):
        idx = query
        ...
        # Modified by SM to add scene_token to metadata
        res = {
            "lidar": {
                "type": "lidar",
                "points": None,
        ...
        pointcloudList = [None for _ in range(1 + np.max([len(sweep_points_list) - 3, 0]).astype(int))]
        if len(sweep_points_list) <= sweepsPerPC:
                pointcloudList[0] = sweep_points_list[:]
        else:
            for timeStep in range(len(pointcloudList)):
                pointcloudList[timeStep] = sweep_points_list[sweepsPerPC * timeStep : (sweepsPerPC * (timeStep + 1))]
        
        pointsList = [np.concatenate(pointcloudList[i], axis=0)[:, [0, 1, 2, 4]] for i in range(len(pointcloudList))]
        resList = [deepcopy(res) for _ in range(len(pointsList))]
        ...
        for idx, res in enumerate(resList):
            ...
            res["lidar"]["points"] = pointsList[idx]
            if 'gt_boxes' in info:
