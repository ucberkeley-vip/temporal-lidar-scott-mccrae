...
from second.pytorch.models import middle, pointpillars, rpn, voxel_encoder, convlstm
...
class VoxelNet(nn.Module):
    ...
    def network_forward(self, voxels, num_points, coors, batch_size):
        ...
        """
        sequenceLength = len(voxels)
        self.start_timer("voxel_feature_extractor")
        voxel_features = []
        for i in range(sequenceLength):
            voxel_features.append(self.voxel_feature_extractor(voxels[i], 
                                    num_points[i], coors[i]))
    ...
    def forward(self, example):
        ...
        """
        voxels = [timeIndex["voxels"] for timeIndex in example]
        # voxels = example["voxels"]
        # num_points = example["num_points"]
        num_points = [timeIndex["num_points"] for timeIndex in example]
        # coors = example["coordinates"]
        coors = [timeIndex["coordinates"] for timeIndex in example]

        batch_anchors = example[0]["anchors"]
        #batch_anchors = [timeIndex["anchors"] for timeIndex in example]
        #batch_size_dev = [batch_anchors[i].shape[0] for i in range(len(batch_anchors))]
        batch_size_dev = batch_anchors.shape[0]
        # Note: now preds_dict_list is a list of preds_dicts
        preds_dict_list = self.network_forward(voxels, num_points, coors, batch_size_dev)

        box_preds_list = preds_dict_list["box_preds"].view(batch_size_dev, -1, self._box_coder.code_size)
        err_msg = f"num_anchors={batch_anchors[-1].shape[1]}, but num_output={box_preds_list[-1].shape[1]}. please check size"
        assert batch_anchors[-1].shape[1] == box_preds_list[-1].shape[1], err_msg
        if self.training:
            loss = self.loss(example[0], preds_dict_list)
            return loss
            # weighted_sum = {}
            # for k in losses[0].keys():
            #     weighted_sum[k] = sum([weight * loss for weight, loss in zip(timestep_weight_loss, [lossdict[k] for lossdict in losses])])
            # return weighted_sum
        else:
            self.start_timer("predict")
            with torch.no_grad():
                res = self.predict(example[0], preds_dict_list)
