...
def train(config_path,
    ...
    step_times = []
    step = start_step
    hidden = [] #Added by SM, contains hidden state
    try:
        while True:
            ...
            for batch in dataloader:
                ...
                # print(batch)
                # Each batch has {batch_size} number of 5-frame sequences
                # The batch is already merged!
                lr_scheduler.step(net.get_global_step())
                examples_torch = []
                for timeIndex in batch:

                    # print(frame)
                    time_metrics = timeIndex["metrics"]
                    timeIndex.pop("metrics")
                    example_timeIndex_torch = example_convert_to_torch(timeIndex, float_dtype)
                    examples_torch.append(example_timeIndex_torch)

                    batch_size = timeIndex["anchors"].shape[0]
                    # batch_size = 3

                # Added by SM, to save memory
                batch = None

                ret_dict = net_parallel(examples_torch)
                print("ran through net!")
                hidden = [] # Reset LSTM hidden state
                ...
                if global_step % steps_per_eval == 0:
                    ...
                    for timeframe in iter(eval_dataloader):
                        hidden = []
                        for example in timeframe:
                            example = example_convert_to_torch(example, float_dtype)
                            curDetections = net(example)
                        detections += curDetections
                        prog_bar.print_bar()
